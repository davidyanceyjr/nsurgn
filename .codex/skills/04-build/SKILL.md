...
7. **Report implementation status.** Label the result `CODE COMPLETE`, `PARTIALLY COMPLETE`, or `BLOCKED` with concise evidence. Claim `CODE COMPLETE` only when every in-scope implementation item is done and required code, configuration, migrations, and generated artifacts are internally consistent. A plan-only output cannot receive `CODE COMPLETE`; blocked or deferred in-scope work requires `PARTIALLY COMPLETE` or `BLOCKED`. Code-complete status is an implementation claim only and does not imply functional verification, merge readiness, release readiness, security, performance, or production readiness.
...
* Avoid treating secure coding basics, local tests, or code-complete status as broader readiness claims.
